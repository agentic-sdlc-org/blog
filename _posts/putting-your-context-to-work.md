...
First, identity. It says what this thing is authoritative about, and most of that you already wrote: the provenance summary (the vision, the vocabulary, the known edges) is the identity the skill needs. Lift it. Name the knowledge, do not restate it.

Second, the trigger surface. The `description` at the top of the file is what a model semantically matches against when it decides which skill to load. It is not decorative prose, it is an interface. Concrete beats abstract every time: "why did my bulk job only process 250" will route a request that "use this for bulk questions" never catches. And the negative half matters just as much. The "do NOT use for" carve-outs are what keep two neighboring skills from fighting over the same request. Ours says things like: not this one for the per-item approval decision, that belongs to the workflow skill. You will not know all the carve-outs on day one. You discover them in use, when the wrong skill fires, and you write them back in. That is context-debt discipline applied one level up.

Third, routing. A trained context is not one document, it is a set of lenses: domain model, standard pattern, data model, anti-patterns, decision criteria, and the rest. All of it is accessible all the time, but "accessible" is not "read in the right order". So the front door funnels. Our bulk-actions skill routes a PRD writer to requirements patterns and decision criteria first, and a DBA to the data model. Nothing is hidden. You are sequencing attention, spending the first and most-anchored reads on the material most likely to answer the question at hand.
