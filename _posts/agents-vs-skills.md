...
excerpt: "We kept having the same argument: is that a skill or an agent? And never finishing it. So we took a trip through how Anthropic and the industry define each, and came back with a definition we can actually build on."
...
There is an argument we keep having at Verusen. It doesn't happen in a meeting with an agenda. It happens in the last five minutes of a standup, in a Slack thread that was supposed to be about something else entirely. Someone says the word *agent*. Someone else says, gently, "isn't that just a skill?" And we're off.
...
Here's where the fog rolls in, and where I think most of our arguments are actually born.
...
To make an **agent**, you assemble four things:
- **a system prompt** that tells it who it is and what it's for
- **a set of tools** it's allowed to use
- **a context window** it thinks inside of
- **a loop** that keeps it going until the work is done.

In Claude Code, the lightest way to spin one up is a Markdown file under `.claude/agents/`. A name, a description, the tools it may touch, and its instructions. Ask Claude Code to tackle something big and it can hand a slice to one of these, which goes off and works with its *own* context window and hands back a result. 
...
Then I went looking for the different types of *skills*, and there aren't any. Not really. A skill can carry more or less. Some are pure instructions, some ship scripts, some bundle reference docs. And skills can be scoped to a person, a project, or a shared plugin. But those are variations in *what a skill carries and where it lives*, not different *kinds* of thing. Every skill is the same species: packaged knowledge, waiting to be loaded.
...
Here's the sentence I wish I'd had at the start of every argument:
